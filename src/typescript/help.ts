...
async function helpMSG(client, message: Message) {
...
async function getCMD(client, message: Message, input) {
  ...
  const em = new MessageEmbed().setTitle(message.author.username).setThumbnail(client.user.avatarURL()).setTimestamp();
	...
	if (!cmd) {
		return message.channel.send({ embeds: [em.setColor('#ff0000').setDescription(info)] });
  }
