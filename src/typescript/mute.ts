...
export default {
    ...
    description:
        'אם אתה לא מוד  OP אין לך שום סיבה להשתמש בזה, תקרא לסיני לעזרה',
    execute: async (bot: Client, msg, args, ms, mc) => {
        // creating a person variable
        var person = msg.guild.member(
            msg.mentions.users.first() || msg.guild.members.get(args[1])
        );
        if (!person) return;

        var mainrole = msg.guild.roles.cache.find(
            (role) => role.name === 'MEMBER'
        );
        var muterole = msg.guild.roles.cache.find(
            (role) => role.name === 'MUTE'
        );
        var policerole = msg.guild.roles.cache.find(
            (role) => role.name === 'POLICE'
        );
        var shingrole = msg.guild.roles.cache.find(
            (role) => role.name === 'שינגעלינג'
        );
        var shing = person.roles.cache.find(
            (role) => role.name === 'שינגעלינג'
        );
        var OP = person.roles.cache.find((role) => role.name === 'OP');

        // check if the member has OP role.
        if (!msg.member.roles.cache.find((r) => r.name === 'OP'))
        ...
        if (!muterole)
            return msg.channel.send('סורי, לא מצאתי את הרול של המיוט');
